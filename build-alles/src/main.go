// multi-service – Asynchrone Microservice-Stub generiert von dredger
package main
...
import (
	...
	"multi-service/async/cli"
	"multi-service/async/config"
	"multi-service/async/handler"
	"multi-service/async/logger"
	"multi-service/async/tracing"
	"multi-service/async/utils"
)
...
func main() {
	...
	if cfg.TracingEnabled {
		tracer := tracing.InitJaegerTracer("Account Service")
		defer tracing.ShutdownTracer(tracer)
	}

	// 4) NATS-Verbindung
	opts := []nats.Option{nats.Name("multi-service")}
