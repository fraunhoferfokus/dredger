...
port := env_var_or_default("ASYNC_SERVICE_PORT_NB", "9090")
...
generate:
	dredger generate OpenAPI.yaml -o . -f -n async-service
...
sbom: sbom-build
	curl -X "POST" "http://dependency-track-api.${DEPENDENCY_TRACK_SERVER}/api/v1/bom" \
		...
		-F "projectName={{program}}/async-service" \
		-F "bom=@sbom.json"

docker-build:
	env GOOS=linux GOARCH=amd64 go build && \
	docker build -t async-service . && \
	rm -rf async-service

docker-run:
	docker run --rm -p 8080:8080 --env-file=.env async-service
